...
int mbedtls_ecdsa_test(void)
{
    ...
    /* show keypair */
    mbedtls_ecp_point_write_binary(&ctx.grp, &ctx.Q, MBEDTLS_ECP_PF_UNCOMPRESSED, &qlen, ecdsa_buf, sizeof(ecdsa_buf));
    dlen = mbedtls_mpi_size(&ctx.d);
    mbedtls_mpi_write_binary(&ctx.d, ecdsa_buf + qlen, dlen);
    dump_buf(ecdsa_buf, qlen + dlen);
    ...
    mbedtls_mpi_write_binary(&r, ecdsa_buf, rlen);
    mbedtls_mpi_write_binary(&s, ecdsa_buf + rlen, slen);
    dump_buf(ecdsa_buf, rlen + slen);
