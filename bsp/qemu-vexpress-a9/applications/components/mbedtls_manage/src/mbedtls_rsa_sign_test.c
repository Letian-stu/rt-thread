...
static void dump_rsa_key(mbedtls_rsa_context *ctx)
{
    ...
    printf("\n  +++++++++++++++++ rsa keypair +++++++++++++++++\n\n");
    mbedtls_mpi_write_string(&ctx->N , 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("N: %s\n", rsa_sign_buf); 

    mbedtls_mpi_write_string(&ctx->E , 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("E: %s\n", rsa_sign_buf);

    mbedtls_mpi_write_string(&ctx->D , 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("D: %s\n", rsa_sign_buf);

    mbedtls_mpi_write_string(&ctx->P , 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("P: %s\n", rsa_sign_buf);

    mbedtls_mpi_write_string(&ctx->Q , 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("Q: %s\n", rsa_sign_buf);

    mbedtls_mpi_write_string(&ctx->DP, 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("DP: %s\n", rsa_sign_buf);

    mbedtls_mpi_write_string(&ctx->DQ, 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("DQ: %s\n", rsa_sign_buf);

    mbedtls_mpi_write_string(&ctx->QP, 16, rsa_sign_buf, sizeof(rsa_sign_buf), &olen);
    printf("QP: %s\n", rsa_sign_buf);
    printf("\n  +++++++++++++++++ rsa keypair +++++++++++++++++\n\n");
}

static void dump_buf(uint8_t *rsa_sign_buf, uint32_t len)
{
    int i;
